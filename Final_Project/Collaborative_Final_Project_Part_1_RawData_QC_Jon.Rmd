...
## Fastqc
Now, I will move into the directory and link the fastq files, activate the environment and run `fastqc` in parallel
```{bash, eval=FALSE}
...
Next I will run [multiqc](https://multiqc.info/) to aggregate the qc info.
...
Now, I will copy the multiqc report `multiqc.html` and the `multiqc_data` directory to my BIO_594 Repository
```{bash, eval=FALSE}
cp -r /home/BIO594/Final_Project/QC/multiqc* ~/repos/BIO_594/Final_Project
```

The mutliqc.json file is big, so we need to use [git LFS](https://git-lfs.com/) to handle the file
...
Now, the report and data files are accessible on github.  To view the full interactive report, download the `multiqc_report.html` file and the whole `multiqc_data` directory and open the `multiqc_report.html` with a web browser.


We can also export images of some of the data.

### Sequence Counts
![Sequence Counts](./Collaborative_Final_Project_Part_1_RawData_QC_Jon_files/fastqc_sequence_counts_plot.png)
This shows a bar graph of the read counts per individual.  This has raw and trimmed reads in it.  You can see that there is a large spread of variance across samples.  Also, for RADseq, don't be alarmed by the presenence of "duplicate reads" this is an approximation and has to do with the fact that RADseq is only sampling at RAD loci.  It is not a random, shotgun sequencing approach.

### Sequence Quality
![Sequence Quality](./Collaborative_Final_Project_Part_1_RawData_QC_Jon_files/fastqc_per_base_sequence_quality_plot.png)
...
![Sequence Quality](./Collaborative_Final_Project_Part_1_RawData_QC_Jon_files/fastqc_per_sequence_quality_scores_plot.png)
This graph looks at the average quality scores per sequence.  I ran this on both raw and trimmed reads.  They look good!

### Per Sequence GC Content

![Sequence GC](./Collaborative_Final_Project_Part_1_RawData_QC_Jon_files/fastqc_per_sequence_gc_content_plot.png)
